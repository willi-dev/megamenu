	...
	class DF_Megamenu {
	  ...
	  //   }
		...
		function df_megamenu_load_style() {
			 ...
			 */
			wp_register_style( 'bootstrap_css', get_template_directory_uri() . '/inc/df-core/asset/css/bootstrap.min.css', array(), '3.3.6', 'all' );
		...
		function df_megamenu_load_script() {
			 ...
			 * temporary load bootstrap
			 */
			wp_register_script( 'bootstrap_js', get_template_directory_uri() . '/inc/df-core/asset/js/bootstrap.min.js', array('jquery'), '3.3.6', true );
			...
			foreach ($items as &$item) {
				...
				if($megadropdown_menu_cat != ''){
					...
					if(sizeof($sub_cat) == 0){ // if cat has no sub-cat with posts
						...
						$new_item->title .= '<a data-target="#df-pane-'.esc_attr( $megadropdown_menu_cat ).'-'.esc_attr( $no_item ).'">All</a>';
						$new_item->title .= '</li>';
						foreach($sub_cat as $sc){
							$new_item->title .= '<li class="">';
								$new_item->title .= '<a data-target="#df-pane-'.esc_attr( $sc->cat_ID ).'-'.esc_attr( $no_item ).'" class="">'.$sc->cat_name.'</a>';
							$new_item->title .= '</li>';
						}
						$new_item->title .= '</ul>';
						$new_item->title .= '</div>';
